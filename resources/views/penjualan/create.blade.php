                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-orange-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Margin</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <template x-for="(item, index) in items" :key="index">
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <select x-model="item.idmargin_penjualan" :name="'items['+index+'][idmargin_penjualan]'" 
                                            @change="updateMarginPersen(index)" required
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                                        <option value="">Pilih Margin</option>
                                        @foreach($margins as $margin)
                                            <option value="{{ $margin->idmargin_penjualan }}" data-persen="{{ $margin->persen }}">
                                                {{ $margin->persen }}%
                                            </option>
                                        @endforeach
                                    </select>
...
function penjualanForm() {
    return {
        ...
        updateMarginPersen(index) {
            const select = document.querySelectorAll('select[name*="idmargin_penjualan"]')[index];
            const selectedOption = select.options[select.selectedIndex];
            const persen = selectedOption.getAttribute('data-persen');
            this.items[index].margin_persen = parseFloat(persen || 0);
        },
        
        calculateMargin(item) {
            const harga = parseInt(item.harga_satuan || 0);
            const persen = parseFloat(item.margin_persen || 0);
