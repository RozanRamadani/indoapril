<x-layout>
    <x-slot:title>Edit Margin Penjualan - IndoApril</x-slot:title>
    
    <x-slot:header>
        <x-header title="Edit Margin Penjualan" subtitle="Mengubah data margin keuntungan penjualan" />
    </x-slot:header>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input type="number" name="persen" id="persen" value="{{ old('persen', $margin->persen) }}" 
                        ...
                        <div>
                            ...
                            <select name="status" id="status" 
                                ...
                                <option value="0" {{ old('status', $margin->status) == '0' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="iduser" id="iduser" 
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->iduser }}" {{ old('iduser', $margin->iduser) == $user->iduser ? 'selected' : '' }}>
